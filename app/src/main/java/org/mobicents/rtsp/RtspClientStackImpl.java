...
public class RtspClientStackImpl implements RtspStack {
    ...
    private String TAG = RtspClientStackImpl.class.getSimpleName();
    ...
    public void start() {
        ...
        Log.d(TAG, "Mobicents RTSP Client started and bound to " + bindAddress.toString());
    }
...
//            logger.info("Mobicents RTSP Client Stop complete");
            Log.d(TAG, "Mobicents RTSP Client Stop complete");
        }
    ...
    public void sendRquest(RtspRequest rtspRequest) {
        ...
        if (channel == null || (channel != null && !channel.isConnected())) {
            ...
            Log.d(TAG, "rtspRequest Host  : " + rtspRequest.getHost());
            Log.d(TAG, "rtspRequest port : " + rtspRequest.getPort());
//            future = bootstrap.connect(new InetSocketAddress(rtspRequest.getHost(), rtspRequest.getPort()));
            future = bootstrap.connect(new InetSocketAddress(rtspRequest.getHost(), rtspRequest.getPort()));
        }
